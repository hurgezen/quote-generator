...
const CreateQuote = () => {
    ...
    setQuoteData(prev => {
      const newLineItems = prev.lineItems.map((item, i) => {
        if (i === index) {
          ...
          let updatedItem = { ...item, [field]: value, [`${field}Numeric`]: numericValue };
          console.log('Updating item:', updatedItem);
          
          updatedItem.dealerTotal = calculateDealerTotal(updatedItem);
          console.log('Recalculated dealerTotal:', updatedItem.dealerTotal);
          
          updatedItem.profitMargin = calculateProfitMargin(updatedItem);
          console.log('Recalculated profitMargin:', updatedItem.profitMargin);
  ...
  const calculateProfitMargin = useCallback((item) => {
    ...
    console.log('calculateProfitMargin:', { cost, revenue, amount, percentage });
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            onChange={(e) => handlePercentageInput(e, index, 'priceAdjustment.percentage')}
                            placeholder="%"
                          />
                          <input
                            type="text"
                            ...
                            onBlur={() => handleBlur(index, 'priceAdjustment.amount')}
                            placeholder="Amount"
                          />
                        </div>
                      </div>
