...
const CreateQuote = () => {
  ...
  const calculateDealerTotal = useCallback((item) => {
    ...
    console.log('calculateDealerTotal:', { quantity, dealerPrice, subtotal, taxesTotal, total });
    return total;
  }, []);

  const calculateProfitMargin = useCallback((item) => {
    const cost = item.dealerPriceNumeric || 0;
    ...
    setQuoteData(prev => {
      const newLineItems = prev.lineItems.map((item, i) => {
        if (i === index) {
          ...
          if (field.startsWith('priceAdjustment.')) {
          ...
          console.log('Before adjustment:', { 
            originalSalePriceNumeric: updatedItem.originalSalePriceNumeric,
            salePriceNumeric: updatedItem.salePriceNumeric,
            priceAdjustment: updatedItem.priceAdjustment
          });
          
          updatedItem.salePriceNumeric = calculateAdjustedSalePrice(updatedItem);
          updatedItem.salePrice = formatCurrency(updatedItem.salePriceNumeric, prev.currency);
  ...
  }, [calculateAdjustedSalePrice, calculateDealerTotal, calculateProfitMargin, formatCurrency]);
                ...
                {quoteData.lineItems.map((item, index) => (
                  <div key={index} className="mb-4 p-4 bg-gray-700 rounded-lg">
                    ...
                    <div className="grid grid-cols-6 gap-4 mb-2">
                      ...
                      <div>
                      <label htmlFor={`priceAdjustment-${index}`} className="block text-sm font-medium text-gray-300 mb-1"></label>
                      ...
                      <div>
                      <label htmlFor={`profitMargin-${index}`} className="block text-sm font-medium text-gray-300 mb-1"></label>
                        <div className="flex gap-2">
                          <div className="w-1/4">
                        ...
                        <div key={taxIndex} className="flex items-center gap-2 mb-2">
                          <div className="relative w-1/4">
                            <input
                              ...
                              onChange={(e) => handlePercentageInput(e, index, `customsTaxes[${taxIndex}].percentage`)}
                            />
